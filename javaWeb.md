...
### 访问者模式是为了将稳定的数据结构和对数据结构的变化多端的操作解耦

### class文件结构

### ClassLoader是一种父优先的等级加载机制，意思就是加载过的抛弃，没加载过的先询问上一级，递归操作，直到获得反馈说这个类没加载过也不应该由上级加载，则可以加载

### ClassLoader方法：
  - findClass(String): 定位要加载的类
  - defineClass(byte[], int, int): 解析Class对象
  - resolveClass(Class<?>): 实例化对象
  - loadClass(String): 获取class对象

### tomcat中servlet的加载都是由WebappLoaderClass来加载的，其加载规则如下：
  1. 

### JVM体系结构：
  - ClassLoader类加载器
  - 执行引擎：可以简单理解为java栈和局部变量区以及对它们进行的操作集合
  - 内存区：有方法区（static，class对象等全局数据区域），java堆（对象实体），PC寄存器（指向java栈的当前栈帧），本地方法区
  - 本地方法接口

### JVM中的方法区和java堆是所有线程共享的

### JIT技术是指JVM会在执行程序时记录某个方法的执行次数，如果执行次数达到一个阀值（客户端一般1500次，服务器端一般是10000次）则JIT会编译这个方法为本地代码以提高执行效率

### java堆大小在JVM启动时就一次向操作系统申请，一旦分配完成则大小就固定

### JVM内存分配策略
  - 栈：和线程绑定，存放基本数据类型对对象引用，速度较快，在编译时确定内存大小，加载时系统一次性把内存分配，当方法结束时分配的内存空间也跟着回收。
  - 堆：运行时数据区，存放对象的实际内容，运行时动态的完成内存的分配，等对象不在被使用时回收，回收由垃圾收集器（GC）负责

### GC主要完成两件事情
  - 正确检测出垃圾对象：
    - 不再被活动对象引用的对象就可以被回收了，活动对象指能够被根对象集合到达的对象
      - 根对象集合：
        - 方法中局部变量区的对象引用
        - java操作栈中的对象引用
        - 常量池中的对象引用
...
###  hotspot使用基于分代的垃圾收集方式，年轻对象正常扫描，老油条减少扫描
  - Young区：又分为Eden区和Survivor区，所有新创建对象都放到这个区，区满后触发minor GC将该区还存活对象放入Survivor区，同时将另一边Survivor区中的数据移动过来，这样保证任何时刻都有一个Survivor区是空的
  - Old区：Survivor区满后触发minor GC仍然存活的对象，如果Survivor区中的对象足够老也存放过来，如果Old区也满了则触发full GC
  - Perm区：存放类的class对象，该区的垃圾回收也是由full GC触发的